...
import { getAIThoughts, subscribeAIThoughts } from '@/lib/aiThoughtsLog';
...
export default function HUDBar() {
  ...
  const [lastAIThought, setLastAIThought] = useState('');

  // Subscribe to AI thoughts for the live indicator
  useState(() => {
    const unsub = subscribeAIThoughts(() => {
      const thoughts = getAIThoughts();
      if (thoughts.length > 0) setLastAIThought(thoughts[0].label);
    });
    return unsub;
  });
  ...
  return (
    <>
      ...
      {/* AI Thoughts toggle — always visible at bottom-right */}
      <div className="fixed bottom-3 right-3 z-40 flex items-center gap-2">
        {lastAIThought && !showAIPanel && (
          <div className="px-3 py-1.5 rounded-lg bg-teal/10 border border-teal/20 max-w-[200px] animate-fade-in">
            <p className="text-[10px] text-teal/70 truncate">🤖 {lastAIThought}</p>
          </div>
        )}
        <button
          onClick={() => setShowAIPanel(!showAIPanel)}
          className={`w-10 h-10 rounded-full flex items-center justify-center text-lg transition-all cursor-pointer ${
            showAIPanel ? 'bg-teal/20 border-2 border-teal' : 'bg-black/60 border border-white/15 hover:bg-black/80'
          }`}
          title="AI Thoughts"
        >
          🤖
